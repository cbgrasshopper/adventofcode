...
const parseExp = /(\D+) (\d+),(\d+)\D+(\d+),(\d+)/;
const directions = input.split('\n').map(line => {
  const matches = line.match(parseExp);
  return {
    'instruction': matches[1],
    'startCoords': [parseInt(matches[2]),parseInt(matches[3])],
    'endCoords': [parseInt(matches[4]),parseInt(matches[5])]
  }
})

const lights = new Array(1000);
for (var i=0; i<1000; i++) {
  let newArray = new Array(1000);
  newArray.fill(false);
  lights[i] = newArray;
}
...
directions.forEach(direction => {
  for (let row = direction.startCoords[0]; row <= direction.endCoords[0]; row++) {
    for (let col = direction.startCoords[1]; col <= direction.endCoords[1]; col++) {
      switch (direction.instruction) {
        ...
        case 'toggle':
          ...
          break;
        default:
          console.log(`Unexpected instruction encountered: ${direction.instruction}`);
      }
    }
  }
});
...
console.log(lights.reduce((prevRowCount, row) => prevRowCount + row.reduce((prevCount, light) => prevCount + light, 0), 0));
