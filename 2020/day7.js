const input = `faded plum bags contain 5 wavy cyan bags.
...
const topLevelComponentsExp = /(?<adj>\S+) (?<color>\S+) bags contain (?<contents>(\d+ \S+ \S+ bags?[,\.]\s?)+|no other bags\.)\n/g;
const containedBagsExp = /((?<qty>\d+) (?<adj>\S+) (?<color>\S+) bags?[,\.])/g;

const getBag = (adjective, colorName) => {
  const bagId = `${adjective} ${colorName}`;
  let bag = bagCollection.find(b => b.id == bagId);
  if (typeof(bag) == 'undefined') {
    bag = {id: bagId, adj: adjective, color: colorName, parents: [], children: []};
    bagCollection.push(bag);
  }
  return bag;
}

const getTopLevelParents = (parentCollection, bag) => {
  for (const parent of bag.parents) {
    parentCollection.add(parent.id);
    getTopLevelParents(parentCollection, parent);
  }
}

const getDescendantCount = (bag) => {
  let runningTotal = 0;
  for (const child of bag.children) {
    runningTotal += child.qty + child.qty * getDescendantCount(child.bag);
  }
  return runningTotal;
}

const matches = [...input.matchAll(topLevelComponentsExp)];
const testMatches = [...testInput.matchAll(topLevelComponentsExp)];

let bagCollection = [];

for (const match of matches) {
  const topLevelBag = getBag(match.groups.adj, match.groups.color);
  const submatches = [...match.groups.contents.matchAll(containedBagsExp)];
  for (const submatch of submatches) {
    const containedBag = getBag(submatch.groups.adj, submatch.groups.color);
    containedBag.parents.push(topLevelBag);
    const quantity = parseInt(submatch.groups.qty);
    topLevelBag.children.push({qty: quantity, bag:containedBag});
  }
}

const shinyGoldBag = bagCollection.find(b => b.id == 'shiny gold');

let topLevelParents = new Set();

getTopLevelParents(topLevelParents, shinyGoldBag);

console.log([...topLevelParents].length);

console.log(getDescendantCount(shinyGoldBag));