...
const anyYeses = input
  .split(`\n\n`)
  .map((group) =>
    group
      .replaceAll(/\s/g, "")
      .split("")
      .reduce((prev, letter) => prev.add(letter), new Set())
  )
  .reduce((prev, set) => (prev += set.size), 0);

  console.log(`Any yeses: ${anyYeses}`);

const allYeses = input
  .split("\n\n")
  .map((group) => group.split("\n").map((answers) => answers.split("")))
  .map((group) =>
    group.reduce(
      (prev, individualAnswer) =>
        prev.filter((answer) => individualAnswer.includes(answer)),
      "abcdefghijklmnopqrstuvwxyz".split("")
    )
  )
  .reduce((prev, answers) => (prev += answers.length), 0);

console.log(`All yeses: ${allYeses}`);
